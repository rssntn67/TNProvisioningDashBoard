...
public class SistemiInformativiTab extends RequisitionTab {
    ...
    private class NodeFilter extends RequisitionNodeFilter {
        ...
        public NodeFilter(Object o, Object c1, Object c3, Object c4,
                Object c5, Object c6) {
            super(o);
            if (c1 != null) {
        ...
        public boolean passesFilter(Object itemId, Item item) {
            ...
            return (super.passesFilter(itemId, item) && (needle1 == null
                    || needle1.equals(node.getServerLevelCategory()[0]))
                    && (needle2 == null
                            || needle2.equals(node.getServerLevelCategory()[1]))
                    && (needle3 == null
                            || needle3.equals(node.getManagedByCategory()))
                    && (needle4 == null
                            || needle4.equals(node.getNotifCategory()))
                    && (needle5 == null
                            || needle5.equals(node.getOptionalCategory()))
                    && (needle6 == null
                            || needle6.equals(node.getProdCategory())));
        }
    ...
    public SistemiInformativiTab() {
        ...
        for (String[] serverLevel : DashBoardUtils.m_server_levels) {
            ...
            m_serverLevelSearchComboBox.setItemCaption(serverLevel,
                                                       serverLevel[0] + " - "
                                                               + serverLevel[1]);
        }
        ...
        for (String[] categories : DashBoardUtils.m_server_levels) {
            ...
            m_serverLevelComboBox.setItemCaption(categories, categories[0]
                    + " - " + categories[1]);
        }
...
//		notifComboBox.setRequiredError("E' necessario scegliere una categoria per le notifiche");
        ...
        m_secondaryIpAddressTable.addGeneratedColumn("Delete",
                                                     new ColumnGenerator() {
                                                         ...
                                                         public Object generateCell(
                                                             ...
                                                             button.addClickListener(new ClickListener() {
                                                                 ...
                                                                 public void buttonClick(
                                                                     ...
                                                                     logger.info("Deleted Secondary ip/service: "
        ...
        m_addSecondaryServiceButton.addClickListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                if (m_secondaryIpTextBox.getValue() != null
                        && m_serviceComboBox.getValue() != null) {
                    String ip = m_secondaryIpTextBox.getValue();
                    if (DashBoardUtils.hasInvalidIp(ip)) {
                        Notification.show("Add secondary ip/service, Failed",
                                          "Invalid Ip address: " + ip,
                                          Type.WARNING_MESSAGE);
                        logger.warning("Added Secondary ip/service Failed: Invalid ip address"
                                + ip);
                        return;
                    }
                    ...
                    logger.info("Added Secondary ip/service: " + ip + "/"
                            + service);
                }
            }
        });
        ...
        m_editorFields.bind(m_serverLevelComboBox,
                            DashBoardUtils.SERVER_LEVEL_CATEGORY);
        ...
        m_editorFields.bind(m_prodComboBox,
                            DashBoardUtils.SERVER_PROD_CATEGORY);
        m_editorFields.bind(m_notifComboBox, DashBoardUtils.NOTIF_CATEGORY);
        m_editorFields.bind(m_managedByComboBox,
                            DashBoardUtils.SERVER_MANAGED_BY_CATEGORY);
        m_editorFields.bind(m_optionalGroup,
                            DashBoardUtils.SERVER_OPTIONAL_CATEGORY);
        ...
        m_editorFields.bind(m_operatingSystem,
                            DashBoardUtils.OPERATINGSYSTEM);
        ...
        if (node.getServiceMap() != null) {
            for (BasicInterface ip : node.getServiceMap().keySet()) {
                for (String service : node.getServiceMap().get(ip)) {
                    if (ip.getIp().equals(node.getPrimary())
                            && service.equals("ICMP"))
                        continue;
    ...
    public SistemiInformativiNode addBean() {
        return m_requisitionContainer.addBeanAt(0,
                                                new SistemiInformativiNode("notSavedHost"
                                                        + newHost++, DashBoardUtils.SI_REQU_NAME)).getBean();
    }
