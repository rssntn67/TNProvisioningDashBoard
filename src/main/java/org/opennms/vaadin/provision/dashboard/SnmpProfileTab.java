...
public class SnmpProfileTab extends DashboardTab {
    ...
    public SnmpProfileTab() {
        ...
        getRightHead().setComponentAlignment(m_removeSnmpButton,
                                             Alignment.MIDDLE_LEFT);
        getRightHead().setComponentAlignment(m_saveSnmpButton,
                                             Alignment.MIDDLE_CENTER);
        getRightHead().setComponentAlignment(m_addSnmpButton,
        ...
        snmpSearchComboBox.addValueChangeListener(new Property.ValueChangeListener() {
            ...
            public void valueChange(ValueChangeEvent event) {
                ...
                if (snmpSearchComboBox.getValue() != null)
                    m_snmpContainer.addContainerFilter(new Compare.Equal("name",
                                                                         snmpSearchComboBox.getValue()));
            }
        });
        ...
        m_snmp_name.addValidator(new RegexpValidator("^[A-Za-z][A-Za-z\\-_0-9]*$",
                                                     "il profilo snmp deve iniziare con un carattere alfabetico"));
        ...
        m_saveSnmpButton.addClickListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                try {
                    ...
                    ce.printStackTrace();
                    logger.warning("Save Snmp Profile Failed: "
                            + ce.getMessage());
                    Notification.show("Save Snmp Profile Failed",
            ...
            public void buttonClick(ClickEvent event) {
                ...
                if (m_snmpContainer.removeItem(m_snmpTable.getValue())) {
                    try {
                        ...
                        uoe.printStackTrace();
                        logger.warning("Delete Snmp Profile Failed: '" + snmp
                                + "'. Error message: "
                                + uoe.getLocalizedMessage());
                        Notification.show("Delete Snmp Profile Failed: '"
                                + snmp + "'.", uoe.getLocalizedMessage(),
                                          Type.ERROR_MESSAGE);
                        return;
                    ...
                    logger.warning("Cannot Found Snmp Profile to Delete: '"
                            + snmp + "'.");
                    Notification.show("Cannot Found Snmp Profile to Delete: '"
                            + snmp + "'.",
                                      "Snmp Profile not found onq SqlContainer",
                                      Type.ERROR_MESSAGE);
                }
            }
        });
    ...
    class DuplicatedSnmpProfileValidator implements Validator {
        ...
        public void validate(Object value) throws InvalidValueException {
            ...
            if (data.getName() != null)
                return;
            logger.info("DuplicatedSnmpProfileValidator: validating Snmp Profile: "
                    + snmp);
            if (m_snmpContainer.getSnmpProfileMap().containsKey(snmp))
                throw new InvalidValueException("DuplicatedSnmpProfileValidator: trovato un duplicato della profilo snmp: "
                        + snmp);
        }
    }
