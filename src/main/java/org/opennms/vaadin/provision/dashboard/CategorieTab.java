...
public class CategorieTab extends DashboardTab {
    ...
    public CategorieTab() {
        ...
        getRightHead().setComponentAlignment(m_removeCatButton,
                                             Alignment.MIDDLE_LEFT);
        getRightHead().setComponentAlignment(m_saveCatButton,
                                             Alignment.MIDDLE_CENTER);
        getRightHead().setComponentAlignment(m_addCatButton,
            ...
            public void buttonClick(ClickEvent event) {
                if (dnsDomainTextBox.getValue() != null) {
                    ...
                    try {
                        ...
                        logger.info("Added Dns Domain: "
                                + dnsDomainTextBox.getValue());
                        Notification.show("Adding", "Dns Domain"
                                + dnsDomainTextBox.getValue() + " Saved",
                        ...
                        e.printStackTrace();
                        logger.warning("Add Dns Domain Failed: "
                                + e.getLocalizedMessage());
                        Notification.show("Add Dns Domain Failed",
                                          e.getLocalizedMessage(),
                                          Type.ERROR_MESSAGE);
        ...
        delDomainButton.addClickListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                if (dnsDomainsDeleteComboBox.getValue() != null
                    ...
                    try {
                        m_domainContainer.commit();
                        logger.info("Deleted Dns Domain: "
                                + dnsDomainsDeleteComboBox.getValue());
                        Notification.show("Delete", "Dns Domain"
                                + dnsDomainsDeleteComboBox.getValue()
                                + " Deleted", Type.HUMANIZED_MESSAGE);
                        dnsDomainsDeleteComboBox.removeAllItems();
                        domainComboBox.removeAllItems();
                        for (Object domain : m_domainContainer.getItemIds()) {
                            dnsDomainsDeleteComboBox.addItem(domain);
                        ...
                        e.printStackTrace();
                        logger.warning("Delete Dns Domain Failed: "
                                + e.getLocalizedMessage());
                        Notification.show("Delete Dns Domain Failed",
                                          e.getLocalizedMessage(),
                                          Type.ERROR_MESSAGE);
                    } catch (SQLException e) {
                        logger.warning("Delete Dns Domain Failed: "
        ...
        networkCatSearchComboBox.addValueChangeListener(new Property.ValueChangeListener() {
            ...
            public void valueChange(ValueChangeEvent event) {
                ...
                if (networkCatSearchComboBox.getValue() != null)
                    m_catContainer.addContainerFilter(new Compare.Equal("networklevel",
                                                                        networkCatSearchComboBox.getValue()));
                if (notifCatSearchComboBox.getValue() != null)
                    m_catContainer.addContainerFilter(new Compare.Equal("notifylevel",
                                                                        notifCatSearchComboBox.getValue()));
                if (threshCatSearchComboBox.getValue() != null)
                    m_catContainer.addContainerFilter(new Compare.Equal("thresholdlevel",
                                                                        threshCatSearchComboBox.getValue()));
                if (catSearchComboBox.getValue() != null)
                    m_catContainer.addContainerFilter(new Compare.Equal("name",
                                                                        catSearchComboBox.getValue()));
            }
        });
        ...
        m_name_cat.addValidator(new RegexpValidator("^[A-Z][A-Za-z\\-0-9]*[A-Za-z0-9]+$",
                                                    "la categoria deve iniziare con una maiuscola e contenere codici alfanumerici"));
        ...
        m_saveCatButton.addClickListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                try {
                    ...
                    ce.printStackTrace();
                    logger.warning("Save Categoria Failed: "
                            + ce.getMessage());
                    Notification.show("Save Categoria Failed",
            ...
            public void buttonClick(ClickEvent event) {
                ...
                if (m_catContainer.removeItem(m_catTable.getValue())) {
                    try {
                        ...
                        uoe.printStackTrace();
                        logger.warning("Delete Categoria Failed: '" + cat
                                + "'. Error message: "
                                + uoe.getLocalizedMessage());
                        Notification.show("Delete Categoria Failed: '" + cat
                                + "'.", uoe.getLocalizedMessage(),
                                          Type.ERROR_MESSAGE);
                        return;
                    ...
                    logger.warning("Cannot Found Categoria to Delete: '" + cat
                            + "'.");
                    Notification.show("Cannot Found Categoria to Delete: '"
                            + cat + "'.",
                                      "Categoria not found onq SqlContainer",
                                      Type.ERROR_MESSAGE);
                }
            }
        });
    ...
    class DuplicateCatValidator implements Validator {
        ...
        public void validate(Object value) throws InvalidValueException {
            ...
            if (data.getName() != null)
                return;
            logger.info("DuplicatedCatValidator: validating categoria: "
                    + cat);
            if (m_catContainer.getCatMap().containsKey(cat))
                throw new InvalidValueException("DuplicatedCatValidator: trovato un duplicato della categoria: "
                        + cat);
        }
    }
    ...
    class DuplicatedDnsValidator implements Validator {
        ...
        public void validate(Object value) throws InvalidValueException {
            ...
            if (m_domainContainer.getDomains().contains(dns))
                throw new InvalidValueException("DuplicatedDnsValidator: trovato un duplicato del domainio: "
                        + dns);
        }
    }
    ...
    class ValidDnsValidator implements Validator {
        ...
        public void validate(Object value) throws InvalidValueException {
            ...
            if (DashBoardUtils.hasInvalidDnsBind9Label(dns))
                throw new InvalidValueException("ValidDnsValidator: dominio non valido: "
                        + dns);
        }
    }
